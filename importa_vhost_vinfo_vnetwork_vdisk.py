...
def normalizar_nomes_colunas(df: pd.DataFrame):
    ...
    mapeamento_colunas = {
        ...
        # Adicionar outros mapeamentos aqui se necessário
    }
...
def ler_sheet(path: str, sheet: str, cols: list) -> pd.DataFrame:
    try:
        ...
        # <<< CHAMADA DA FUNÇÃO normalizar_nomes_colunas
...
def importar_arquivo(file_path: str):
    ...
    try:
        ...
        if not vhost_df.empty:
            print(f"[INFO] Processando {len(vhost_df)} registros da aba vHost...")
            ...
            print("[AVISO] Nenhum dado em vHost.")

        # 2) VMs
        if not vinfo_df.empty:
            print(f"[INFO] Processando {len(vinfo_df)} registros da aba vInfo...")
            ...
            print("[AVISO] Nenhum dado em vInfo.")
        
        # 3) Redes
        if not vnetwork_df.empty:
